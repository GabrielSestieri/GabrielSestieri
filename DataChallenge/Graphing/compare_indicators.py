...
def get_data(country, indicator1, indicator2, date):
    ...
    with open(country+"_"+indicator1+".csv", "a+") as output_file:
        counter1 = 0
        while counter1 <= 30:
            url = "https://covidmap.umd.edu/api/resources?indicator=" + indicator1 + "&type=smoothed&country=" + country + "&date=" + date1
    ...
    with open(country+"_"+indicator2+".csv", "a+") as output_file:
        ...
        while counter2 <= 30:
            #print(counter2)
            url = "https://covidmap.umd.edu/api/resources?indicator=" + indicator2 + "&type=smoothed&country=" + country + "&date=" + date2
            response = requests.get(url).text 
            if response != "Internal Server Error":
                jsonData = json.loads(response)
                toCSV = jsonData.get('data')
